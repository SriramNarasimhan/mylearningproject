	...
	function getParameterByName(name, url) {
	  ...
	  $(".workFlowDataForm_submit").click(function(e){
		...
		function checkProgress() {
		    $.getJSON('/dotcom/monitorprogress?taskIdentity=s3', function(progress) {
		        ...
		        var percent = totalProcessed/totalItems;
		    	
		        $('#progress').text(percent + "%");
		        $('#progress .bar').width(percent);
		
		        if (parseInt(progress) < 100) {
		            setTimeout(checkProgress, 5000); // Checks again after one second.
		        }
		    });
		}
